...
import 'homepages/meal_suggestions_page.dart'; // NEW
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadDailyGoal() async {
    ...
    try {
      ...
      final goal = prefs.getInt(_prefKey);
      if (!mounted) return;
  ...
  Future<void> _loadTodayTotal() async {
    ...
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString('$_entriesKeyPrefix${_todayKey()}');
      int total = 0;
      if (raw != null) {
  ...
  Future<void> _loadAttivitaScelteOggi() async {
    ...
    try {
      final prefs = await SharedPreferences.getInstance();
      final key = 'attivita_selezionate_${_todayKeyCompact()}';
      ...
      setState(() => _attivitaScelteOggi = parsed);
    } finally {
  ...
  // -------- Calorie card trailing (grafico + badge affiancati) --------
        ...
        child: Padding(
          ...
          child: ListView(
            children: [
              ...
              Builder(
                builder: (context) {
                  const baseColor = Color.fromARGB(255, 98, 160, 204);
                  ...
                  return Card(
                    ...
                    child: Padding(
                      padding: const EdgeInsets.all(9), // inset
                      child: Material(
                        color: Colors.transparent,
                        child: Ink(
                          decoration: BoxDecoration(
                            color: bg, // riempimento soft
  ...
  // ---- trailing attività fisica (icona + timer) ----
